...
// переменная для хранения id ингредиента из модели
// если окажется undefined, форма не даст добавить ингредиент
let idIng;
...
function Ingredients() {
    ...
    const dropdown = (e) => {
        if (e.target.classList.contains('form__item-list')) {
            ...
            idIng = e.target.getAttribute('data-id');
        }
    };
    // Добавление элемента из инпута
    // Обязательная проверка айдишника ингредиента
    const addIngredient = (e) => {
        if(nameIngredient.value && cantidad.value && typeof idIng !== 'undefined' && idIng !== 0) {
            const data = getValue();
            ...
            elem.innerHTML = `<span> ${data.name} ${data.value}${data.units}</span> <span class="form__field-item-delete"></span>
                             ...
                             <input id="dimensionIngredient_${cur}" name="dimensionIngredient" type="hidden" value="${data.units}">`;
            idIng = 0;
            cur++;
